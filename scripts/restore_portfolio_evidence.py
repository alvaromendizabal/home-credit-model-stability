...
def evidence_client(role_arn: str, region: str) -> Any:
    ...
    require(os.environ.get("GITHUB_REPOSITORY") == "alvaromendizabal/home-credit-model-stability", "unexpected OIDC repository")
    require(os.environ.get("GITHUB_REF") in {"refs/heads/main", "refs/heads/feat/validated-model-release"}, "untrusted publication branch")
    url = os.environ["ACTIONS_ID_TOKEN_REQUEST_URL"]
    require(url.startswith("https://"), "OIDC requires HTTPS")
    response = requests.get(url, params={"audience": "sts.amazonaws.com"}, headers={"Authorization": "Bearer " + os.environ["ACTIONS_ID_TOKEN_REQUEST_TOKEN"]}, timeout=20)
    response.raise_for_status()
    token = response.json()["value"]
    credentials = boto3.client("sts", region_name=region).assume_role_with_web_identity(RoleArn=role_arn, RoleSessionName="home-credit-aggregate-review", WebIdentityToken=token, DurationSeconds=900)["Credentials"]
    return boto3.client("s3", region_name=region, aws_access_key_id=credentials["AccessKeyId"], aws_secret_access_key=credentials["SecretAccessKey"], aws_session_token=credentials["SessionToken"])
...
def restore_evidence(root: Path, client: Any, bucket: str, logger: RunLogger) -> None:
    ...
    for entry in entries:
        ...
        require(path.suffix == ".json" and path.is_relative_to(root / "reports"), "only aggregate JSON can be published")
        with StageTimer(logger, "restore_" + path.stem, heartbeat_seconds=15):
            ...
            require(isinstance(payload, dict) and payload.get("schema_version") == 1, "unsupported evidence schema")
            json.dumps(payload, allow_nan=False)
            atomic_write(path, raw)
            logger.event("aggregate_evidence_restored", file=path.name, sha256=entry["sha256"], bytes=len(raw))
...
def main() -> None:
    ...
    region = "us-west-2"
    restore_evidence(root, evidence_client(role, region), bucket, RunLogger("evidence-review", root / "logs"))
