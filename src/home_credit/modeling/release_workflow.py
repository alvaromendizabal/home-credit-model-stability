...
from home_credit.modeling.inference import (
...
def publish_verified(store: ExperimentStore, path: Path, relative: str) -> dict[str, Any]:
    ...
    require(
        remote is not None and sha256_bytes(remote[0]) == sha256_file(path), "S3 read-back mismatch"
    )
...
def restore_snapshot(root: Path, plan: dict[str, Any], store: ExperimentStore) -> FeatureSnapshot:
    ...
    for number, block in enumerate(manifest["blocks"], 1):
        ...
        store.logger.event(
            "release_feature_verified", completed=number, total=len(manifest["blocks"])
        )
    return FeatureSnapshot.load(
        directory, expected_manifest_sha256=digest, expected_protocol_sha256=plan["protocol_sha256"]
    )


def validate_policy(
    root: Path, plan: dict[str, Any], store: ExperimentStore
) -> tuple[tuple[FeatureRef, ...], dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    require(
        plan["automatic_kaggle_submission"] is False and plan["automatic_submission_csv"] is False,
        "the release cannot perform the owner's export action",
    )
    ...
    require(
        plan["holdout_fit_weeks"] == [outer["development_week_min"], outer["development_week_max"]],
        "development scope changed",
    )
    require(
        plan["holdout_evaluation_weeks"]
        == [outer["validation_week_min"], outer["validation_week_max"]],
        "holdout scope changed",
    )
    require(plan["final_refit_weeks"] == [0, 91], "all-label refit scope changed")
    selected = plan["selection"]
    require(
        set(plan["components"]) == set(selected["weights"]) and len(plan["components"]) == 2,
        "frozen component budget changed",
    )
    path = store.root / "inputs/selection.json"
    store.download(selected["key"], path, selected["sha256"])
    evidence = read_object(path)
    require(
        evidence["outer_holdout_touched"] is False and evidence["smoke"] is False,
        "invalid development selection",
    )
    require(evidence["selected_candidate"] == selected["candidate"], "winner changed")
    require(
        set(evidence["selected_weights"]) == set(selected["weights"]), "ensemble members changed"
    )
    for name, weight in selected["weights"].items():
        require(
            math.isclose(weight, evidence["selected_weights"][name], rel_tol=0, abs_tol=1e-12),
            "ensemble weight changed",
        )
    source = read_object(root / "configs/model_selection.json")
    store.download(
        source["study"]["key"], store.root / "inputs/tuning.json", source["study"]["sha256"]
    )
    study = read_object(store.root / "inputs/tuning.json")
    require(
        study["complete"] is True and study["outer_holdout_touched"] is False,
        "invalid tuning study",
    )
    ...
    for name, component in plan["components"].items():
        ...
        require(
            state["identity"]["feature_manifest_sha256"] == plan["feature_manifest_sha256"],
            "source feature lineage changed",
        )
        require(
            state["identity"]["validation_protocol_sha256"] == plan["protocol_sha256"],
            "source validation lineage changed",
        )
        require(
            frozen_iterations(state) == component["num_boost_round"],
            "tree budget does not match development evidence",
        )
    ...
    features = tuple(
        FeatureRef(**row)
        for row in read_object(root / "configs/benchmark_features.json")["selected_features"]
    )
...
def fit_phase(
            ...
            canonical_json_bytes(
                {
                    ...
                    "rounds": plan["components"][name]["num_boost_round"],
                    "seed": plan["seed"],
    ...
    for name in names:
        ...
        if label in state["stages"]:
            ...
            require(
                record["fit_spec_sha256"] == specifications[name],
                "cached fit specification changed",
            )
            require(
                record["fit_weeks"] == weeks and record["fit_rows"] == expected_rows,
                "cached fit scope changed",
            )
    ...
    for name in pending:
        ...
        receipt = fit_frozen_model(
            ...
            rounds=component["num_boost_round"],
            seed=plan["seed"],
            threads=plan["threads"],
            path=path,
            logger=store.logger,
        ...
        state = commit_stage(
            ...
            {
                **receipt,
                "fit_spec_sha256": specifications[name],
                "fit_git_commit": subprocess.check_output(
                    ["git", "rev-parse", "HEAD"], text=True
                ).strip(),
                "fit_weeks": weeks,
                "model": model_member,
                "encoder": encoder_member,
            },
        )
        store.logger.event(
            "release_model_completed",
            phase=phase,
            model=name,
            completed=sum("/" in key for key in state["stages"]),
            total=plan["max_new_model_fits"],
        )
        gc.collect()
    del matrix, target
    gc.collect()
...
def evaluate_holdout(
    ...
    intent = {
        ...
        "weights": plan["selection"]["weights"],
        "models": {name: row["model"]["sha256"] for name, row in members.items()},
        "encoders": {name: row["encoder"]["sha256"] for name, row in members.items()},
        "fit_weeks": plan["holdout_fit_weeks"],
        "evaluation_weeks": plan["holdout_evaluation_weeks"],
    }
    ...
    if existing is None:
        ...
        require(
            json.loads(existing[0]) == intent,
            "the holdout is already bound to a different frozen model; no retuning is allowed",
        )
    store.logger.event(
        "holdout_intent_verified", intent_sha256=sha256_bytes(canonical_json_bytes(intent))
    )
    ...
    with StageTimer(store.logger, "evaluate_frozen_outer_holdout", heartbeat_seconds=15):
        ...
        require(
            frame.height == protocol["outer_holdout"]["validation_rows"],
            "holdout row count changed",
        )
        probability = predict_components(
        ...
        report = evaluation_report(
            frame, probability, expected_weeks=list(range(weeks[0], weeks[1] + 1))
        )
        report.update(
            schema_version=1,
            scope="One frozen outer-holdout evaluation; not a leaderboard result",
            fit_weeks=plan["holdout_fit_weeks"],
            evaluation_weeks=weeks,
            calibration="none",
            intent=intent,
            release_identity=state["identity"],
            evaluated_utc=iso_utc(),
            evaluated_model_phase="development",
            all_label_model_evaluated=False,
        )
        predictions = store.root / "holdout/predictions.parquet"
        predictions.parent.mkdir(parents=True, exist_ok=True)
        frame.select("case_id", "target", "WEEK_NUM").with_columns(
    ...
    return commit_stage(
        store, lease, state, "holdout", {"report": report_member, "predictions": predicted_member}
    )


def prepare_raw_schema(
    root: Path, plan: dict[str, Any], protocol: dict[str, Any], store: ExperimentStore
    ...
    schema: dict[str, Any] = {
        "schema_version": 1,
        "recipe_sha256": sha256_file(root / "configs/feature_recipe.json"),
        "blocks": {},
    }
    for source in sources:
        ...
        schema["blocks"][f"{source.family}_depth{source.depth}"] = {
            "semantic": asdict(semantic),
            "allowed_absent_columns": sorted(set(semantic.predictors) - observed),
        }
...
def verify_raw_parity(
    ...
    with StageTimer(store.logger, "raw_test_feature_parity", heartbeat_seconds=15):
        ...
        actual = raw_test_frame(
            raw_directory, records, features, schema, directory / "feature_recipe.json"
        )
        reference = pl.scan_parquet(
            next(b.path for b in snapshot.test_blocks if b.family == "base")
        ).select("case_id")
        for block in snapshot.test_blocks:
            names = [f.name for f in features if f.block == block.name]
            if names:
                reference = reference.join(
                    pl.scan_parquet(block.path).select(["case_id", *names]),
                    on="case_id",
                    how="left",
                    validate="1:1",
                )
        expected = (
            reference.select(["case_id", *[f.name for f in features]]).sort("case_id").collect()
        )
        assert_frame_equal(actual, expected, check_exact=False, rel_tol=1e-6, abs_tol=1e-8)
        store.logger.event(
            "raw_test_parity_passed",
            rows=actual.height,
    ...
    return commit_stage(
        ...
        "raw_parity",
        {
            ...
            "rows": actual.height,
            "schema": publish_verified(
                store, directory / "raw_schema.json", "parity/raw_schema.json"
            ),
            "features": publish_verified(store, frame_path, "parity/features.parquet"),
        },
    )
...
def assemble_bundle(
    ...
    prediction = predict_components(
        transform(actual, features, encoder),
        features,
        {name: directory / item["path"] for name, item in models.items()},
        plan["selection"]["weights"],
    )
    ...
    bundle = {
        "schema_version": 1,
        "phase": "all_labels",
        "fit_weeks": plan["final_refit_weeks"],
        "calibration": "none",
        ...
        "release_identity": state["identity"],
        "feature_manifest_sha256": plan["feature_manifest_sha256"],
        "holdout_report_sha256": state["stages"]["holdout"]["report"]["sha256"],
        "holdout_score_applies_to_this_model": False,
        "files": files,
    ...
    with StageTimer(store.logger, "verify_packaged_raw_inference", heartbeat_seconds=15):
        raw_prediction = predict_raw(
            directory,
            raw_directory,
            store.root / "inference_batches",
            expected_bundle_sha256=sha256_file(directory / "bundle.json"),
            logger=store.logger,
            batch_rows=3,
        )
        require(
            raw_prediction["case_id"].tolist() == actual["case_id"].to_list(),
            "packaged raw prediction coverage changed",
        )
        require(
            bool(np.allclose(raw_prediction["score"], prediction, rtol=0, atol=1e-12)),
            "packaged raw inference differs from frozen feature scoring",
        )
        resumed = predict_raw(
            directory,
            raw_directory,
    ...
    members = [
        publish_verified(store, directory / item["path"], f"bundle/{item['path']}")
        for item in files
    ]
        ...
        for path in sorted(directory.rglob("*")):
            if path.is_file():
                item = zipfile.ZipInfo(
                    path.relative_to(directory).as_posix(), date_time=(1980, 1, 1, 0, 0, 0)
                )
    ...
    return commit_stage(
        ...
        state,
        "bundle",
        {"manifest": manifest_member, "files": members, "archive": archive_member},
    )
...
def execute(root: Path, bucket: str, *, preflight_only: bool = False) -> dict[str, Any]:
    ...
    dependencies = [
        "configs/model_release.json",
        "configs/model_benchmark.json",
        "configs/model_selection.json",
        "configs/benchmark_features.json",
        "configs/validation_protocol.json",
        "uv.lock",
        "src/home_credit/modeling/release.py",
        "src/home_credit/modeling/data.py",
        "src/home_credit/modeling/encoding.py",
        "src/home_credit/metrics/classification.py",
        "src/home_credit/metrics/stability.py",
    ]
    versions = {
        name: importlib.metadata.version(name)
        for name in ["numpy", "pandas", "polars", "pyarrow", "lightgbm", "scikit-learn"]
    }
    dependency_lock = tomllib.loads((root / "uv.lock").read_text())
    for name, version in versions.items():
        require(
            {p["version"] for p in dependency_lock["package"] if p["name"] == name} == {version},
    ...
    store = ExperimentStore(
        client, bucket, f"home-credit-model-stability/model-release/{key}", work, logger
    )
    with (work / "run.lock").open("a") as lock:
        ...
        try:
            with WriterLease(store) as lease:
                ...
                state = verify_raw_parity(
                    root, snapshot, features, plan, protocol, state, store, lease
                )
                if preflight_only:
                    logger.event(
                        "model_release_preflight_completed",
                        new_model_fits=0,
                ...
                state = fit_phase(
                    "development",
                    plan["holdout_fit_weeks"],
                    protocol["outer_holdout"]["development_rows"],
                    snapshot,
                    features,
                    plan,
                    parameters,
                ...
                state = fit_phase(
                    "all_labels",
                    plan["final_refit_weeks"],
                    protocol["observed_temporal_structure"]["train_rows"],
                    snapshot,
                    features,
                    plan,
                    parameters,
                ...
                state = assemble_bundle(
                    root, snapshot, features, plan, protocol, state, store, lease
                )
                state["complete"] = True
                state["revision"] += 1
                state["completed_utc"] = iso_utc()
                state["reporting_git_commit"] = subprocess.check_output(
                    ["git", "rev-parse", "HEAD"], text=True
                ).strip()
                store.commit(state)
                logger.event(
                    "model_release_completed",
                    release_key=key,
                    new_model_fit_budget=4,
...
def main() -> int:
    ...
    print(
        "MODEL_RELEASE_PREFLIGHT_COMPLETED" if args.preflight_only else "MODEL_RELEASE_COMPLETED",
        flush=True,
    )
    return 0
